...
export class GoodReadStore {
  ...
  toggleItemRead(id: number, isRead: boolean) {
    this.backendService.markItem(id, isRead).subscribe(rsp => {
      const findIndexToUpdate = this.reads.findIndex(read => read.id === id);
      this.reads[findIndexToUpdate] = rsp;
    });
  }

  @action
  updateRead(read: GoodRead) {
    this.backendService.editItem(read).subscribe((updatedRead: GoodRead) => {
      const itemIndexUpdated = this.reads.findIndex(
        read => read.id === updatedRead.id
      );
